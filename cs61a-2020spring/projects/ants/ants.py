    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if isinstance(self, ContainerAnt) and not isinstance(place.ant, ContainerAnt) and self.can_contain(place.ant):
                ...
                place.ant = self
            elif not isinstance(self, ContainerAnt) and isinstance(place.ant, ContainerAnt) and place.ant.can_contain(self):
                self.place = place
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        def find_nearest_bee(place, transition):
            if place == beehive:
                return None
            if transition < self.min_range:
                return find_nearest_bee(place.entrance, transition + 1)
            ...
            if bees:
                ...
                return find_nearest_bee(place.entrance, transition + 1)
        return find_nearest_bee(self.place, 0)
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor - amount <= 0:
            total_damage = self.damage + self.armor
        else:
            total_damage = amount
...
# END Problem 13
    ...
    def action(self, gamestate):
        ...
        if not self.truth:
            ...
            ants = []
            while search_place.exit:
                if search_place.ant:
                    ants.append(search_place.ant)
                    if isinstance(search_place.ant, ContainerAnt) and search_place.ant.contained_ant:
                        ants.append(search_place.ant.contained_ant)
                search_place = search_place.exit
            for ant in ants:
                if ant not in self.buffed_ant:
                    self.buffed_ant.append(ant)
                    ant.damage *= 2
        # END Problem 13
    ...
    def reduce_armor(self, amount):
        ...
        if self.truth and self.armor <= 0:
            bees_win()
        # END Problem 13
...
class Bee(Insect):
    ...
    # OVERRIDE CLASS ATTRIBUTES HERE
    is_watersafe = True
    direction = True   # to show the bee goes to exit or entrance
    scared = False     # to show whether the bee has already been scared
    ...
    def action(self, gamestate):
        ...
        # BEGIN EC
        "*** YOUR CODE HERE ***"
        """if not self.direction and (self.scare_time < 2 and (self.original_time == 1000 or (gamestate.time - self.original_time < 2))):
        if not self.direction and self.scare_time < 2:
            if self.original_time == 1000:
                self.original_time = int(gamestate.time)
            destination = self.place.entrance
            self.scare_time += 1
            if self.scare_time == 2:
                self.direction = True
...
def apply_effect(effect, bee, duration):
    ...
    def Action(gamestate):
        ...
        if duration > 0:
            ...
            duration -= 1
        else:      # when the duration is no more than one, change the direction of the bee, just to make sure the bee do not goes to entrance again
            bee.direction = True
