...
def scheme_eval(expr, env, _=None):
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        operand_func = lambda exp: scheme_eval(exp, env)
        operands = operands.map(operand_func)   # make every operand is been evaluated
...
y    LambdaProcedure(Pair('x', nil), Pair(Pair('+', Pair('x',od Pair(2, nil))), nil), <Global Frame>)
    """
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        value = scheme_eval(bind.rest.first, env)
