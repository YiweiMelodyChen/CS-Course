...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    for i in range(1, length):
        if (diff_function(user_word, valid_words[i], limit) < min_diff) or valid_words[i] == user_word:
            tag = valid_words[i]
...
def sphinx_swap(start, goal, limit):
    ...
    if limit == 0:
        return 1
    return (start[0] != goal[0]) + sphinx_swap(start[1:], goal[1:], limit - (start[0] != goal[0]))
    # END PROBLEM 6
...
def report_progress(typed, prompt, id, send):
    ...
    for i in range(length_typed):
        if typed[i] != prompt[i]:
            break
        tag = i
    if length_typed > 0 and typed[0] == prompt[0]:
        tag += 1
    pro = tag / length_prompt
    send({'id': id, 'progress': pro})
    return pro
    # END PROBLEM 8
...
def time_per_word(times_per_player, words):
    ...
    for x in times_per_player:
        for i in range(len(x) - 1):
            x[i] = x[i + 1] - x[i]
        x.pop()
    # game_list.append(times_per_player)
...
def fastest_words(game):
    ...
    for player in players:
        fastest_list.append([])
    for word in words:
        low_time = time(game, 0, word)
        tag = 0
        for player in players:
            if time(game, player, word) < low_time:
                low_time = time(game, player, word)
                tag = player
        fastest_list[tag].append(word_at(game, word))
    return fastest_list
    # END PROBLEM 10
